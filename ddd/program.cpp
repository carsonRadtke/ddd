...
static void init_scene(std::vector<EntityUPtr> & entities)
{
  ...
  auto sph1 =
      std::make_unique<Sphere>( Sphere( Color( 0, 255, 255 ), 0, 500, 10, 50 ) );
  auto sph2 =
      std::make_unique<Sphere>( Sphere( Color( 255, 0, 255 ), -300, 700, 10, 100 ) );
  auto sph3 =
      std::make_unique<Sphere>( Sphere( Color( 255, 255, 0 ), 600, 100, 0, 500 ) );
